...
Forbidden rule keywords: 'record', 'trigger', 'call', 'find*'.",
    ...
    "play-information-runtime-info": {
      type: "object",
      required: ["desc", "factory", "schema"],
      description: "Describes a single information type or list used by the app. \
If `desc.isList` is true, schema.type must be 'array' and items must reference the corresponding single-item schema in #/definitions (kebab-case). Follow naming convention described in #/definitions/information-schema-guidelines.",
      properties: {
        desc: {
          oneOf: [
            { $ref: "#/definitions/value-object-description" },
    ...
    "information-schema-guidelines": {
      "title": "Information Schema Convention",
      "description": "Guidelines for defining information schemas and corresponding definitions. \
Each information type must have a schema in `types` and a matching definition entry under `#/definitions` with a kebab-case key. \
If `isList` is true, the schema must be of type `array` and its items must `$ref` the corresponding single item definition. \
Example: \
- Crm.Client → #/definitions/crm/client \
- Crm.Clients → #/definitions/crm/clients (array of #/definitions/crm/client)",
      "examples": [
        {
          "types": {
            "Crm.Client": {
              "schema": { "$ref": "#/definitions/crm/client" },
              "desc": { "isList": false }
            },
            "Crm.Clients": {
              "schema": {
