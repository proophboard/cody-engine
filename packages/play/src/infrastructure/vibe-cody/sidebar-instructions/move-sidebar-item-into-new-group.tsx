...
const TEXT = `Move item into a new sidebar group called: `

export const MoveSidebarItemIntoNewGroup: Instruction = {
  text: TEXT,
  icon: <Group />,
  isActive: context => context.focusedElement?.type === "sidebarItem",
  match: input => input.startsWith(TEXT),
  execute: async (input, ctx, dispatch, config) => {
    const groupName = input.replace(TEXT, '').trim();

    if(groupName === '') {
      return {
        cody: `Missing a proper group name. Please try again.`,
    ...
    return {
      cody: `Moved the item into the new group "${groupName}"`
    }
  }
}
