...
import {getBreadcrumbFnFromPlayPage} from "@cody-play/app/layout/PlayBreadcrumb";
...
const PlaySidebarSubMenu = (props: SidebarSubMenuProps) => {
  ...
  const renderNextLevel = (nextLevel: number) => {
    if (nextLevel < matchingPages.length) {
      ...
      return <List disablePadding={true} key={`sidebar-next-level-${nextPage.route}`} sx={{
        ...
        <SubMenuItem key={`sub-menu-item-${nextPage.route}`} page={{...nextPage, breadcrumb: getBreadcrumbFnFromPlayPage(nextPage, config, user)}}/>
...
export default PlaySidebarSubMenu;
